...
export default function RegisterForm() {
  ...
  return (
    <form autoComplete="off" className="border border-gray-400 rounded-xl w-[33vw] shadow-2xl py-8 px-6">
      ...
      {isEmailError ? <p className="text-red-600">Invalid email format</p> : null}
      {isLoginError ? <p className="text-red-600">This username is already taken</p> : null}
      {isPasswordError ? <p className="text-red-600">Password must be 8 or more characters</p> : null}
      <button
  ...
  <div className="flex gap-3 w-full justify-center mt-4">
    <p className="text-gray-400">Already have an Account?</p> <Link to="/login" className="font-bold hover:text-gray-500">Log in</Link>
  </div>
    </form>
